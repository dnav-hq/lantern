...
- **World-class pass (2026-09-18, `docs/audits/world-class-pass-2026-09-18.md`) — ALL FIXED the same day.**
  Scripture full-text search (in-browser scan of the shipped BSB bundle,
  `utils/scriptureSearch.ts`, third section of search); the mobile composer
  carries the verse it is about; a bare highlight renders no empty inline row;
  the highlight picker is capped at half the screen and the verse scrolls
  clear of it; guests get the same top-bar menu as members (Profile, Settings,
  Export, Sign in); the dead Study tab from the library; search's empty state;
  the chapter strip recentres on resize; "--" removed from journey gap notes;
  `utils/doorways.ts` deleted; ESV carries an "online only" tag in the
  switcher. The full map keeps one quiet way in ("The whole map" under the
  card) and no longer shows the dataset-wide confidence legend when framed on
  a chapter. Still a choice for Dennis: whether the full map should adopt the
  card's parchment paint (the parked atlas task) or stay the plain line map.
...
- **Footnotes door (deep-dive rung 1) — the reading surface is BUILT
  ...
  12.7% across the whole ship set, §10b), `FootnoteDoor.tsx` draws the 1px dotted
  underline and opens the note on click, and both reading surfaces
  (`BookDetailPage`, `ReadingMode`) drop their doors while a verse selection is
  live. STILL OPEN: the mobile tap arbitration wants a real device rather than a
  390px emulation (§5.4, §10.3) — nothing else in the brief can settle it. Also
  still open: `BiblePassage`'s `BibleVerse` type does not declare `notes` even
  though every read carries them, so `FootnoteDoor` reads them through one
  documented cast — widen `src/types/index.ts` next time someone is in that file.
...
- **The setting line under a cross-reference — one short objective line saying
  where the reader is about to land, so three references can be told apart.**
  SLICE 1 SHIPPED 2026-09-13 (`docs/proposals/setting-line.md` §9): the BSB's
  own section heading under every connection row, framed as a heading ("Under
  “Faith and Works”") and gated to verses within 10 of it, with the Hebrew
  superscription second in the chain. `scripts/build-setting-lines.mjs` →
  `public/bible/connections/settings.json.gz` (9,393 destination verses, 53.9 KB
  gzipped, 85.0% of the 39,209 reachable rows); the rules are code in
  `src/utils/settingLine.ts` (R3 banned verbs, R4's 12-word cap, R6's reach),
  run over every line and again over the gzip the build writes, and the bundle
  is fetched lazily on first door open, memoized, and outside the PWA precache.
  Nine headings in reach carry a banned verb ("Love Fulfills the Law", "The
  Fulfillment of the Law", "Jesus Teaches at the Feast") and are DROPPED and
  reported rather than shipped. Nothing is generated by a model. STILL OPEN:
  §8's D1 hand pass over the head of the distribution (the 1,000
  most-referenced destinations cover 34.8% of rows; the audited imprecision rate
