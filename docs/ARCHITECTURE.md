...
| One grammar for choosing a verse (2026-09-18) | "Tap a verse" had come to mean three things — select and act (Read), aim the note (Study), select and compose (mobile) — and each new feature attached to whichever meaning it was built against, so the app's identity drifted. The rule, stated once: **choosing a verse always shows two things, what is beneath it and what you can do with it.** The dive-in is a property of the verse, not of a mode: Read shows the one entrance line under the chosen verse, Study shows the same dive-in body open beside the note (the workbench has the room), and a chapter marks in the gutter which verses have depth at all (`utils/chapterDepth.ts`, presence only, on the same facts the entrance opens on). Anything that changes what choosing a verse shows must satisfy this sentence first. |
